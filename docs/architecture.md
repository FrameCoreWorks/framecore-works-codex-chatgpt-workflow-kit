...
`scripts/chatgpt-skill-sources.mjs` regenerates and validates that mapping after source changes. ChatGPT reads only the selected skill sources and creates each native skill through its host-managed Create with chat workflow. Eligible accounts may use `skill-creator` automatically, but repository setup does not require a literal command or visible tool named `$skill-creator`. Codex agent templates, AGENTS files, local manifests, Context, Memory Cache, private paths, and generated workspace state remain outside this route. ChatGPT uses role IDs as temporary responsibilities inside the current task; it does not consume `.codex/agents/*.toml` as persistent agents.
