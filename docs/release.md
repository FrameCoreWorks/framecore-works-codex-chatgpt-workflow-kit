...
`chatgpt:skills:check` validates `CHATGPT_INSTALL.md`, all 37 public skill folders, native UI metadata, install profiles, raw source URLs, and checked-in SHA-256 hashes without modifying source files.
...
2. Confirm `config/chatgpt-skill-sources.json` contains all 37 skills and only intended files.
