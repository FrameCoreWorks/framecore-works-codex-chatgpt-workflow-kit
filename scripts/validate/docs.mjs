...
export function run(ctx) {
  ...
  if (existsSync(chatGptSkillsDoc)) {
    ...
    for (const phrase of ["CHATGPT_INSTALL.md", "config/chatgpt-skills.json", "config/chatgpt-skill-sources.json", "agents/openai.yaml", "ChatGPT > Work", "Use @skill-creator", "Plugins > Skills > Create > Create with chat", "native Skill mention", "needs_user_confirmation", "voice approval does not prove native installation", "estimated cost or `Unknown`", "fresh setup session", "Do not use ChatGPT Memory", "Ask every onboarding question from scratch", "small workflow helpers", "can be refined or expanded later", "npm run chatgpt:skills:check", "npm run chatgpt:skills:sources:update", "setup language", "temporary responsibility", "must not report bulk success", "allow_implicit_invocation: true", "smallest sufficient route", "@workflow-orchestrator", "@pipeline-core", "explicit-only", "all 34"]) {
